...
def feed_data(premise, premise_mask, hypothesis, hypothesis_mask, y_batch, features,
              dropout_keep_prob):
...
def evaluate(sess, premise, premise_mask, hypothesis, hypothesis_mask, y, features):
    batches = next_batch(premise, premise_mask, hypothesis, hypothesis_mask, y, features)
...
def train():
    ...
    premise_dev, premise_mask_dev, hypothesis_dev, hypothesis_mask_dev, y_dev ,features_dev= sentence2Index(arg.devset_path, vocab_dict, embeddings, maxLen=arg.seq_length, write_features=False)
    print('Train size: {} Dev size:{}'.format(len(premise_train), len(premise_dev)))
    ...
    sess.run(model.init_embedding, feed_dict={model.embed_matrix: embeddings})
    ...
    for epoch in range(arg.num_epochs):
        ...
        batches = next_batch(premise_train, premise_mask_train, hypothesis_train, hypothesis_mask_train, y_train, features_train, batchSize=arg.batch_size)
        ...
        for batch in batches:
            ...
            if total_batch % arg.eval_batch == 0:
                ...
                loss_val, acc_val = evaluate(sess, premise_dev, premise_mask_dev, hypothesis_dev, hypothesis_mask_dev, y_dev, features_dev)
                ...
                if acc_val > best_acc_val:
                    ...
                    print_log('Update best model with dev_acc:{}'.format(best_acc_val), file=log)
                    saver.save(sess = sess, save_path = arg.best_path + '_dev_acc_{:.4f}.ckpt'.format(acc_val))
                    improved_flag = '*'
                else:
                    improved_flag = ''
